...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);
    // EmailJS: replace service_id, template_id, public_key with yours
    setTimeout(() => {
      setLoading(false);
  ...
  return (
    <div id="contact" className="xl:mt-12 xl:flex-row flex-col-reverse flex gap-10 overflow-hidden">
      ...
      {/* Right side info panel instead of 3D Earth */}
      <motion.div
        className="xl:flex-1 flex flex-col justify-center gap-8 p-8"
      ...
      >
        <div>
          <p className="text-slate-500 font-mono text-sm mb-1">EMAIL</p>
          <p className="text-white text-lg">abhinavcheepa7@gmail.com</p>
        </div>
        <div>
          <p className="text-slate-500 font-mono text-sm mb-1">PHONE</p>
          <p className="text-white text-lg">+91 7000759702</p>
        </div>
        <div>
          <p className="text-slate-500 font-mono text-sm mb-1">LOCATION</p>
          <p className="text-white text-lg">MITS Gwalior, India</p>
        </div>
        <div>
          <p className="text-slate-500 font-mono text-sm mb-1">GITHUB</p>
